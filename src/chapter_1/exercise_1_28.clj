...
;; be fooled is called the Miller-Rabin test (Miller 1976; Rabin 1980). 
...
;; Modify the `expmod` procedure to signal if it discovers
;; a nontrivial square root of 1, and use this to implement
...
(defn check-nontrivial-sqrt [x n]
  (and (not= x 1)
       (not= x (- n 1))
       (= (mod (square x) n) 1)))
...
(defn test-int [n] (prime? n 7))

(map test-int [561 1105 1729 2465 2821 6601, 7])