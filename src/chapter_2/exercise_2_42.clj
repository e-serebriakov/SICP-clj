...
;; or diagonal). One possible solution is shown in Figure 2.8.
;; 8  Q . . . . . . .
;; 7  . . . . Q . . .
;; 6  . . . . . . . Q
;; 5  . . . . . Q . .
...
;; queen in the kth column is safe with respect to the other queens. 
...
;; row in which to place the queen for the kth column. 
...
;; to each other.)
(def empty-board '())
(defn adjoin-position [r col rest]
  (cons (list r col) rest))

(defn safe? [k positions]
  (let [[[current-row _] & rest-queens] positions]
    (every? (fn [[row col]]
              (let [row-dif (Math/abs (- current-row row))
                    col-dif (Math/abs (- k col))]
                (and
                 (not= current-row row)
                 (not= row-dif col-dif))))
            rest-queens)))
...
(defn draw-board [positions board-size]
  (let [queens-by-col (->> positions
                          (map (fn [[row col]] [col row]))
                          (into {}))
        row->str (fn [row]
                  (->> (range 1 (inc board-size))
                       (map #(if (= row (queens-by-col %)) "Q " ". "))
                       (apply str)
                       (format "%d %s\n" row)))
        col-labels (->> (range 1 (inc board-size))
                       (map #(char (+ 96 %)))
                       (interpose \space)
                       (apply str)
                       (format "  %s"))]
...
(println (draw-board (first (queens 4)) 4))
