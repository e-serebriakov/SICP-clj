...
;; name indicates that, when the procedure is called,
...
(defn same-parity [a & rest]
  (let [filter-fn (if (odd? a) odd? even?)]
    (cons a (filter filter-fn rest))))

(defn same-parity [first & rest]
  (let [nums (cons first rest)]
    (filter #(= (odd? %) (odd? first)) nums)))
